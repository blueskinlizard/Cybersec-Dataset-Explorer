...
print("UNSW-NB15 - FEATURE USEFULNESS & INTERPRETABILITY METRICS (PCA-based)")

FEATURE_GROUPS = {
    'Basic Flow': ['dur', 'spkts', 'dpkts', 'sbytes', 'dbytes', 'rate'],
    'Network Quality': ['sttl', 'dttl', 'sloss', 'dloss', 'sjit', 'djit', 'sload', 'dload'],
    'TCP Features': ['swin', 'dwin', 'tcprtt', 'synack', 'ackdat'],
    'Connection Patterns': ['ct_srv_src', 'ct_srv_dst', 'ct_dst_ltm', 'ct_src_ltm', 
                           'ct_src_dport_ltm', 'ct_dst_sport_ltm', 'ct_dst_src_ltm'],
    'State Features': ['ct_state_ttl', 'ct_flw_http_mthd', 'ct_ftp_cmd'],
    'Binary Indicators': ['is_sm_ips_ports', 'is_ftp_login'],
    ...
    'Engineered - Loss & Jitter': ['has_packet_loss', 'total_loss', 'loss_rate', 
                                    'avg_jitter', 'high_jitter'],
    'Engineered - TCP': ['has_tcp_info', 'window_size_avg', 'has_timing', 'tcp_handshake_time'],
    'Engineered - Scanning': ['is_scanner', 'diverse_ports', 'diverse_src_ports', 
                              'repeated_connection', 'connection_burst'],
    'Engineered - Application': ['is_http', 'http_methods_count', 'is_ftp', 'ftp_commands_count'],
...
available_features = [f for f in all_numeric_features if f in UNSW_NB15.columns]

print(f"\nAnalyzing {len(available_features)} features across {len(FEATURE_GROUPS)} categories")

X = UNSW_NB15[available_features].fillna(0).replace([np.inf, -np.inf], 0)
y = UNSW_NB15['is_attack'].values
...
metrics_combined.to_csv('feature_metrics_unsw_pca.csv', index=False)
print(f"\nMetrics saved to 'feature_metrics_unsw_pca.csv'")

fig, axes = plt.subplots(2, 2, figsize=(16, 12))
fig.suptitle('UNSW-NB15 Feature Analysis (PCA-based Interpretability)', 
...
ax3.set_yticklabels(group_stats_plot.index, fontsize=8)
...
ax4.set_xticklabels(top_10_metrics['feature'], rotation=45, ha='right', fontsize=8)
ax4.legend(fontsize=8, loc='upper right')
ax4.grid(axis='y', alpha=0.3)

plt.tight_layout()
plt.savefig('feature_metrics_visualization_unsw_pca.png', dpi=300, bbox_inches='tight')
print("Visualization saved to 'feature_metrics_visualization_unsw_pca.png'")
